...
static int himax_read_dev_info(struct himax_data *ts)
{
	int ret;
	uint8_t data[4] = { 0 };
				...
				int data_len)
{
	...
	if (ts->pdata->reset) {
		if (ts->debug_log_level)
			dev_info(&ts->client->dev, "reseting the device ...\n");
...
static void himax_work_func(struct work_struct *work)
{
	...
	if (ts->debug_log_level & 0x1) {
		dev_info(&ts->client->dev, "raw data:\n");
	...
	if (buf[20] == 0xFF && buf[21] == 0xFF) {
		...
		if (ts->first_pressed == 1) {
			...
			if (ts->debug_log_level & 0x2)
				dev_info(&ts->client->dev, "End @ %d, %d\n",
					 ts->pre_finger_data[0],
					 ts->pre_finger_data[1]);
		}

		if (ts->debug_log_level & 0x2)
			dev_info(&ts->client->dev, "Finger leave\n");
	} else {
		...
		if (buf[base] == '\n') {
			if (buf[0] == 'w')
				himax_i2c_write(ts->client, ts->himax_command,
						&write_da[0], length);
			dev_info(&ts->client->dev, "CMD: %x, %x, %d\n",
			       ...
			       size_t count)
{
	...
	if (buf[0] == '1' && ts->pdata->reset) {
		...
		dev_info(&ts->client->dev, "Now reset the Touch chip.\n");
		       ...
		       const struct i2c_device_id *id)
{
	...
	if (ts->pdata->regulator_info) {
		ret = himax_power_device(ts, true);
		if (ret) {
			dev_err(&client->dev, "enable of regulators failed\n");
			goto err_regulators_failed;
		}
	}
	...
	ret = himax_read_dev_info(ts);
	if (ret) {
		dev_err(&client->dev, "reading of firmware version failed\n");
		goto err_reading_fw;
	}
	...
	dev_info(&client->dev, "Register input device\n");
	...
	if (client->irq) {
		...
		if (ret == 0)
			dev_info(&client->dev, "irq enabled at qpio: %d\n",
				 client->irq);
		else {
			ts->use_irq = 0;
...
static int himax_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	ts->suspend_mode = 1;
	if (ts->debug_log_level)
		dev_info(&client->dev, "device suspended\n");
...
static int himax_resume(struct i2c_client *client)
{
	...
	if (ts->use_irq)
		enable_irq(client->irq);
	if (ts->debug_log_level)
		dev_info(&client->dev, "device resumed\n");
	return 0;
}
