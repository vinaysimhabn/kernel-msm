...
static int otm8018b_panel_unprepare(struct drm_panel *panel)
{
	...
	ret = regulator_bulk_disable(ARRAY_SIZE(otm8018b->supplies), otm8018b->supplies);
	if (ret < 0)
	...
	gpiod_set_value(otm8018b->bklt_gpio, 0);
		udelay(100);
...
static int otm8018b_panel_prepare(struct drm_panel *panel)
{
	...
	ret = regulator_bulk_enable(ARRAY_SIZE(otm8018b->supplies), otm8018b->supplies);
	if (ret < 0) {
	...
	gpiod_set_value(otm8018b->bklt_gpio, 1);
	mdelay(1);
...
static int otm8018b_panel_add(struct otm8018b_panel *otm8018b)
{
	...
	for (i = 0; i < ARRAY_SIZE(otm8018b->supplies); i++)
		otm8018b->supplies[i].supply = regulator_names[i];

	ret = devm_regulator_bulk_get(dev, ARRAY_SIZE(otm8018b->supplies),
				      otm8018b->supplies);
	if (ret < 0) {
	...
	if (IS_ERR(otm8018b->reset_gpio)) {
		ret = PTR_ERR(otm8018b->reset_gpio);
		dev_err(dev, "cannot get reset-gpios %d\n", ret);
		return ret;
	}

	otm8018b->bklt_gpio = devm_gpiod_get(dev, "bklt-pwm", GPIOD_OUT_LOW);
	if (IS_ERR(otm8018b->bklt_gpio)) {
		ret = PTR_ERR(otm8018b->bklt_gpio);
