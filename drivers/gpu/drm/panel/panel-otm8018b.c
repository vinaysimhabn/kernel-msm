...
static int otm8018b_panel_unprepare(struct drm_panel *panel)
{
	...
	if (otm8018b->backlight_gpio){
		gpiod_set_value(otm8018b->backlight_gpio, 0);
		udelay(100);
	}
...
static int otm8018b_panel_prepare(struct drm_panel *panel)
{
	...
	if (ret < 0)
		return ret;
    if (otm8018b->backlight_gpio) {
		gpiod_set_value(otm8018b->backlight_gpio, 1);
		mdelay(5);
	}
...
static int otm8018b_panel_add(struct otm8018b_panel *otm8018b)
{
	...
	otm8018b->backlight_gpio = devm_gpiod_get(dev, "backlight", GPIOD_OUT_LOW);
	if (IS_ERR(otm8018b->backlight_gpio)) {
		dev_err(dev, "cannot get backlight-gpios %ld\n",
			PTR_ERR(otm8018b->backlight_gpio));
		otm8018b->backlight_gpio = NULL;
	} else {
		gpiod_direction_output(otm8018b->backlight_gpio, 0);
	}
...
static int otm8018b_panel_probe(struct mipi_dsi_device *dsi)
{
	...
	int ret;
    printk("pradeep : otm8018b_panel_probe \n");
	dsi->lanes = 2;
